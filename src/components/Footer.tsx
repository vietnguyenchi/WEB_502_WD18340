...
const Footer = () => {
  return (
    <Box as='footer' bg='gray.300' borderTop='1px solid' borderColor='gray.300' py='2.5rem' fontSize='0.875rem'>
      <Box maxW='8xl' marginX='auto' pb='2rem' mb='1.5rem' px={10} borderBottom='1px solid' borderColor='gray.300'>
        <Flex flexWrap='wrap' alignItems='start' justifyContent='space-between'>
          <Box w={{ base: '100%', sm: '50%', md: 'max-content' }} mb={{ base: '1.5rem', lg: '0' }}>
            ...
            <List paddingLeft={0} lineHeight='2' justifyContent='center'>
          ...
          <Box w={{ base: '100%', sm: '50%', md: 'max-content' }} mb={{ base: '1.5rem', lg: '0' }}>
            ...
            <List paddingLeft={0} lineHeight='2'>
...
const LinkItem = ({ text, isTag = false, tagText }: LinkItemProps) => {
  return (
    <ListItem display='flex'>
      <Link fontWeight='600' href='#' color='rgba(113, 128, 150, 1)' _hover={{ color: 'green.600' }}>
        {text}
      </Link>
      {isTag && (
        <Text
